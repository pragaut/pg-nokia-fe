...
class TowerNotificationDetails extends Wrapper {
    ...
    async componentDidMount() {
        ...
        this.props.getTowerNotificationDetails(0, constants.DEFAULT_ROWS_LIST, undefined, undefined);
...
const mapStateToProps = state => {
    const { towerNotificationDetails, towerNotificationDetail } = state.workingReducerTmc;
    const { alarms, alarm } = state.adminReducer;
    const errorType = state.errorReducer.type;
    const errorMessage = state.errorReducer.error;

    return { towerNotificationDetails, towerNotificationDetail, alarms, alarm, errorType, errorMessage };
};
